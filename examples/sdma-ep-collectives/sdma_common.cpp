...
void SdmaTestEngine::runBenchmark() {
  ...
  // Initialize signal buffers once at the start
  HIP_CHECK(
    hipMemset(barrierSignals_, 0, config_.nBlocks * nPeer * sizeof(uint64_t)));
  HIP_CHECK(
  ...
  for (size_t size = minBytes_; size <= maxBytes_;) {
    ...
    // Don't reset signals - they keep growing across sizes
    // Only clear result buffer before initialization (not after!)
