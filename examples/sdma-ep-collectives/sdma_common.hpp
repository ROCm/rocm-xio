...
__device__ inline void crossGpuBarrier(uint64_t*** remoteBarrierSignals,
  ...
  if (threadIdx.x < static_cast<uint32_t>(nPeer)) {
    ...
    __atomic_store_n(remoteBarrierSignals[threadIdx.x][slotIdx], expectedSignal,
                     __ATOMIC_RELAXED);
    while (__atomic_load_n(&localBarrierSignals[threadIdx.x], __ATOMIC_RELAXED) <
           expectedSignal) {
    }
  }
  __syncthreads();
...
public:
  ...
  // Getters
  const SdmaTestConfig& getConfig() const { return config_; }
...
private:
  ...
  void reportResults(double avgTimeMs, size_t totalBytes,
...
public:
  ...
  virtual void initData(int rank, int nranks, void* inputBuf,
                       void* expectedBuf, size_t nelemsPerRank) = 0;
  ...
  virtual size_t getResultBufferSize(size_t nelemsPerRank, int nranks) const = 0;
  virtual size_t getExpectedBufferSize(size_t nelemsPerRank, int nranks) const = 0;

  // Set pointers to engine-managed resources
  // Called by engine after allocation
  virtual void setBuffers(int* input, int* result,
