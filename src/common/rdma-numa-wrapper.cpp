...
void NUMAWrapper::set_preferred(int node) { funcs_.set_preferred(node); }

int NUMAWrapper::num_configured_nodes() {
  return funcs_.num_configured_nodes();
}

int NUMAWrapper::num_configured_cpus() { return funcs_.num_configured_cpus(); }

int NUMAWrapper::node_of_cpu(int cpu) { return funcs_.node_of_cpu(cpu); }

int NUMAWrapper::max_node() { return funcs_.max_node(); }
