...
int IBVWrapper::is_dmabuf_supported() { return dmabuf_is_supported_; }
...
struct ibv_pd *
IBVWrapper::alloc_parent_domain(struct ibv_context *context,
...
struct ibv_mr *IBVWrapper::reg_mr(struct ibv_pd *pd, void *addr, size_t length,
  ...
  if (is_dmabuf_supported()) {
    ...
    hsa_status_t status =
      hsa_amd_portable_export_dmabuf(addr, length, &fd, &offset);
...
struct ibv_cq_ex *
IBVWrapper::create_cq_ex(struct ibv_context *context,
...
struct ibv_qp *
IBVWrapper::create_qp_ex(struct ibv_context *context,
