...
[[maybe_unused]] static const char *GidPriorityStr[] = {
  "RoCEv1 Link-local",    "RoCEv2 Link-local",
  "RoCEv1 IPv6",          "RoCEv2 IPv6",
  "RoCEv1 IPv4-mapped",   "RoCEv2 IPv4-mapped"
};
...
static bool IsIPv4MappedIPv6(const union ibv_gid &gid) {
  return (gid.global.subnet_prefix == 0 && gid.raw[8] == 0 &&
...
static int AutoSelectGidIndex(struct ibv_context *context, int gidTblLen,
  ...
  for (int i = 0; i < gidTblLen; ++i) {
    ...
    if (IsIPv4MappedIPv6(gid)) {
      currPriority =
        (roceVersion == 2) ? ROCEV2_IPV4 : ROCEV1_IPV4;
    } else if (!LinkLocalGid(gid)) {
      currPriority =
        (roceVersion == 2) ? ROCEV2_IPV6 : ROCEV1_IPV6;
    } else {
      currPriority =
        (roceVersion == 2) ? ROCEV2_LINK_LOCAL : ROCEV1_LINK_LOCAL;
    }
...
static std::vector<IbvDeviceInfo> &GetIbvDeviceList() {
  ...
  if (!isInitialized) {
    ...
    for (int i = 0; i < numIbvDevices; i++) {
          ...
          for (int port = 1; port <= deviceAttr.phys_port_cnt; ++port) {
            ...
            if (portAttr.state == IBV_PORT_ACTIVE) {
              ...
              if (portAttr.link_layer == IBV_LINK_LAYER_ETHERNET) {
                info.isRoce = true;
                info.gidIndex =
                  AutoSelectGidIndex(context, portAttr.gid_tbl_len, port);
                if (info.gidIndex >= 0 &&
                    static_cast<int>(static_cast<GidPriority>(ROCEV2_IPV4)) >= 0) {
      ...
      std::string numaPath =
        "/sys/bus/pci/devices/" + info.busId + "/numa_node";
...
int GetClosestNicToGpu(int gpuIndex, const char *hca_list,
  ...
  std::string excludeList(
    (hca_list && hca_list[0] == '^') ? &hca_list[1] : "");
  std::string includeList(
    (hca_list && hca_list[0] != '^') ? hca_list : "");
  ...
  for (size_t i = 0; i < ibvDeviceList.size(); i++) {
    ...
    auto is_excluded =
      hasExactMatch(excludeList, dev.name) ||
