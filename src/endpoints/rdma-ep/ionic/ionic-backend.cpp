...
using qp_set_gda_fn = int (*)(
    struct ibv_qp *, bool, bool);
...
#define LOAD(field, name)                          \
  if (dlsym_load(ionic_dv.field,                   \
                 ionicdv_lib_handle_, name) != 0)  \
    return -1;

  LOAD(get_ctx, "ionic_dv_get_ctx");
  LOAD(qp_get_udma_idx,
...
void Backend::ionic_create_cqs(int ncqes) {
  ...
  pd_parent_ =
      ibv.alloc_parent_domain(context_, &pattr);
  XIO_CHECK_NNULL(pd_parent_,
  ...
  memset(&ionic_cq_attr, 0,
         sizeof(ionic_cq_attr));
  ionic_cq_attr.comp_mask =
  ...
  struct ibv_cq_ex *cq_ex =
      ionic_dv.create_cq_ex(
          context_, &cq_attr, &ionic_cq_attr);
  if (!cq_ex) {
    fprintf(stderr,
            "rdma_ep::ionic: "
...
void Backend::ionic_initialize_gpu_qp() {
  ...
  host_qp_->ionic_cq_buf_ =
      static_cast<struct ionic_v1_cqe *>(
          dvcq.q.ptr);
  ...
  host_qp_->ionic_sq_buf_ =
      static_cast<struct ionic_v1_wqe *>(
          dvqp.sq.ptr);
