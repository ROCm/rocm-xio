...
/**
 * ionic_dv_pd_get_udma_mask - Get mask of udma pipeline ids of pd or parent
 * domain.
 */
 ...
 *
 * Queues associated with this pd will be restricted to one of the pipelines
 * enabled by the mask at the time of queue creation.
 *
 * Recommended usage is to create a pd, then parent domains of that pd for each
 * different udma mask.  Set the desired udma mask on each parent domain.  Then,
 * create queues associated with the parent domain with the desired udma mask.
 *
 * Alternative usage is to create a pd, and set the desired udma mask prior to
 * creating each queue.  Changing the udma mask of the pd has no effect on
 * previously created queues.
 */
int ionic_dv_pd_set_udma_mask(struct ibv_pd* ibpd, uint8_t udma_mask);

/**
 * ionic_dv_cq_get_udma_mask - Get mask of udma pipeline ids of completion
 * queueue.
 */
...
/**
 * ionic_dv_pd_set_sqcmb - Specify send queue preference for controller memory
 * bar.
 *
 * Send queues associated with this pd will use the controller memory bar
 * according to this preference at the time of queue creation.
 *
 * @param ibpd Protection domain to configure.
 * @param enable Allow the use of the controller memory bar.
 * @param expdb Allow the use of express doorbell optimizations.
 * @param require Require preferences to be met, no fallback.
 */
int ionic_dv_pd_set_sqcmb(struct ibv_pd* ibpd, bool enable, bool expdb,
                          bool require);

/**
 * ionic_dv_pd_set_rqcmb - Specify receive queue preference for controller
 * memory bar.
 *
 ...
 *
 * @param ibpd Protection domain to configure.
 * @param mask IONIC_EXPDB_* bitmap
 */
 ...
 *
 * In GDA mode, when the application calls ibv_post_send() or ibv_post_recv(),
 * the provider writes WQEs in the descriptor ring without ringing the doorbell.
 *
 * To ring the doorbell, after posting the work the application should query to
 * get the doorbell data, and later write that data to the memory mapped
 * doorbell register.
 *
 ...
 *
 * @param ibqp Set GDA mode for this queue pair.
 * @param enable_send Enable GDA mode for the send queue.
 * @param enable_recv Enable GDA mode for the recv queue.
 */
 ...
 *
 * In GDA mode, when the application calls ibv_post_send() the provider writes
 * WQEs in the descriptor ring without ringing the doorbell.  The application
 * should query the doorbell data immediately after posting the work.  The
 * application requests the GPU to fill the source buffers of the data transfer
 * with the result of computation. The application requests the GPU to write the
 * doorbell data to the memory mapped doorbell register immediately when the
 * computation is complete, triggering the data transfer.
 *
 * It is important that the GPU ring the doorbell in sequential order.  If work
 * requests are posted in batches A, B, and C, with respective doorbell data,
 * the data path must not write B or C before A, and must not write C before B.
 * It is ok to skip writing a doorbell, like writing only C, which will trigger
 * the data transfer for all of the work up to that point in the sequence.
 *
 * @param ibqp Get send doorbell data for this queue pair.
 * @param dbdata Output parameter for doorbell data.
 */
 ...
 *
 * In GDA mode, when the application calls ibv_post_recv() the provider writes
 * WQEs in the descriptor ring without ringing the doorbell.  After polling recv
 * completions, the application can immediately re-post the receive buffers
 * without ringing the doorbell. The application should query the doorbell data
 * immediately after posting the buffers. The application requests the GPU
 * consume the data from the receive buffers.  The application requests the GPU
 * to write the doorbell data to the memory mapped doorbell register immediately
 * after the received data is consumed, making the buffers available for the
 * next data transfer.
 *
 * It is important that the GPU ring the doorbell in sequential order.  If work
 * requests are posted in batches A, B, and C, with respective doorbell data,
 * the data path must not write B or C before A, and must not write C before B.
 * It is ok to skip writing a doorbell, like writing only C, which will make
 * buffers available up to that point in the sequence.
 *
 * @param ibqp Get recv doorbell data for this queue pair.
 * @param dbdata Output parameter for doorbell data.
 */
 ...
 *
 * @param ibctx Context CQ will be attached to.
 * @param ex IBV attributes to create the CQ with.
 * @param ionic_ex Vendor-specific attributes to create the CQ with.
 */
