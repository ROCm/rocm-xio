...
void *Backend::mlx5_dv_dlopen() {
  ...
  handle = dlopen(
      RDMA_CORE_LIB_DIR "/libmlx5.so",
      RTLD_LAZY);
#endif
  if (!handle)
    handle = dlopen(
        "libmlx5.so", RTLD_LAZY);
  if (!handle)
    handle = dlopen(
...
int Backend::mlx5_dv_dl_init() {
  ...
  if (dlsym_load(mlx5_dv.init_obj,
                 mlx5dv_handle_,
                 "mlx5dv_init_obj") != 0)
    return -1;
...
void Backend::mlx5_initialize_gpu_qp() {
  ...
  int ret = mlx5_dv.init_obj(
      &obj,
      MLX5DV_OBJ_QP | MLX5DV_OBJ_CQ);
  if (ret != 0) {
    fprintf(stderr,
            "rdma_ep::mlx5: "
  ...
  host_qp_->mlx5_cq_ = gda_mlx5_device_cq(
      static_cast<mlx5_cqe64 *>(dv_cq.buf),
      dv_cq.dbrec);

  host_qp_->mlx5_sq_ = gda_mlx5_device_sq(
      static_cast<gda_mlx5_wqe *>(
          dv_qp.sq.buf),
      dv_qp.dbrec,
      static_cast<gda_mlx5_doorbell *>(
          dv_qp.bf.reg),
