...
struct ernicdv_funcs_t {
  ...
  struct rocm_ernic_dv_umem* (*umem_reg)(struct ibv_context* ctx,
                                         struct rocm_ernic_dv_umem_attr* attr);
  int (*umem_dereg)(struct rocm_ernic_dv_umem* umem);
  uint32_t (*get_cq_id)(struct ibv_cq* cq);
  int (*get_qp_attr)(struct ibv_qp* qp, struct rocm_ernic_dv_qp_attr* attr);
};
