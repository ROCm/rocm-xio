...
#define LOAD(field, name)                          \
  if (dlsym_load(ernic_dv.field,                   \
                 ernicdv_handle_, name) != 0)      \
    return -1;
...
void Backend::ernic_create_cqs(int cqe) {
  ...
  ernic_scq_ = static_cast<ernic_host_cq *>(
      calloc(1, sizeof(ernic_host_cq)));
  ernic_rcq_ = static_cast<ernic_host_cq *>(
      calloc(1, sizeof(ernic_host_cq)));

  create_one_cq(ernic_scq_, context_,
                ernic_dv, cqe, dmabuf, "SCQ");
  XIO_CHECK_NNULL(ernic_scq_->cq,
                  "ernic_dv_create_cq (SCQ)");

  create_one_cq(ernic_rcq_, context_,
                ernic_dv, cqe, dmabuf, "RCQ");
  XIO_CHECK_NNULL(ernic_rcq_->cq,
                  "ernic_dv_create_cq (RCQ)");
...
void Backend::ernic_create_qps(int sq_length) {
  ernic_qp_ = static_cast<ernic_host_qp *>(
      calloc(1, sizeof(ernic_host_qp)));

  uint32_t max_swr =
      static_cast<uint32_t>(sq_length);
  uint32_t max_ssge = 1;
  ...
  auto *sq_umem =
      ernic_dv.umem_reg(context_, &sq_ua);
  XIO_CHECK_NNULL(sq_umem,
                  "ernic_dv_umem_reg (SQ)");

  void *rq_ptr = nullptr;
  if (posix_memalign(&rq_ptr, pgsz,
                     ernic_qp_->rq_len)) {
    fprintf(stderr,
            "rdma_ep::ernic: "
  ...
  auto *rq_umem =
      ernic_dv.umem_reg(context_, &rq_ua);
  XIO_CHECK_NNULL(rq_umem,
                  "ernic_dv_umem_reg (RQ)");

  memset(&ernic_qp_->attr, 0,
         sizeof(ernic_qp_->attr));
  ...
  ernic_qp_->uar_qp_offset =
      qp_attr.uar_qp_offset;
  ernic_qp_->uar_cq_offset =
      qp_attr.uar_cq_offset;
