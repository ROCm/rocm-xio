...
int Backend::init() {
  if (!ibv.is_initialized) {
    fprintf(stderr,
            "rdma_ep: IBV wrapper not initialized. Cannot init backend.\n");
    return -1;
  }
  ...
  if (!qp_) {
    fprintf(stderr, "rdma_ep: QP creation failed.\n");
    return -1;
  }
...
void Backend::open_dv_libs() {
  ...
  if (provider_ == Provider::UNKNOWN &&
    ...
    if (mlx5_dv_dl_init() == 0) {
      ...
      fprintf(stderr, "rdma_ep: MLX5 DV library "
                      "loaded.\n");
    } else {
      fprintf(stderr, "rdma_ep: MLX5 DV library "
                      "not available.\n");
  ...
  if (provider_ == Provider::UNKNOWN &&
    ...
    if (ionic_dv_dl_init() == 0) {
      ...
      fprintf(stderr, "rdma_ep: IONIC DV library "
                      "loaded.\n");
    } else {
      fprintf(stderr, "rdma_ep: IONIC DV library "
                      "not available.\n");
...
#if defined(GDA_ERNIC)
  if (provider_ == Provider::UNKNOWN &&
      (requested == Provider::UNKNOWN || requested == Provider::ROCM_ERNIC)) {
    if (ernic_dv_dl_init() == 0) {
      provider_ = Provider::ROCM_ERNIC;
      fprintf(stderr, "rdma_ep: ERNIC DV library "
                      "loaded.\n");
    } else {
      fprintf(stderr, "rdma_ep: ERNIC DV library "
                      "not available.\n");
...
void Backend::open_ib_device() {
  ...
  fprintf(stderr,
          "rdma_ep: Opened device: %s (vendor=0x%x, part=0x%x, fw=%s)\n",
  ...
  if (gid_index_ < 0) {
    fprintf(stderr,
            "rdma_ep: Warning: could not auto-select GID index, using 0.\n");
    gid_index_ = 0;
  }
...
void Backend::create_queues() {
  ...
  if (provider_ == Provider::IONIC) {
    ...
    if (!cq_) {
      fprintf(stderr, "rdma_ep: ionic CQ creation failed"
...
void* Backend::pd_alloc_device_uncached(struct ibv_pd* pd, void* pd_context,
  ...
  hipError_t err = hipExtMallocWithFlags(&dev_ptr, size,
...
int Backend::connect_to_peer(const DestInfo& remote) {
  ...
  fprintf(stderr,
          "rdma_ep: Connected to peer (QPN=%u, GID=%02x%02x:...:%02x%02x)\n",
          remote.qpn, remote.gid.raw[0], remote.gid.raw[1], remote.gid.raw[14],
          remote.gid.raw[15]);
  return 0;
}
...
int Backend::register_data_buffer(void* buf, size_t size) {
  ...
  struct ibv_pd* mr_pd = pd_parent_ ? pd_parent_ : pd_;
  heap_mr_ = ibv.reg_mr_host(mr_pd, buf, size, access);
  if (!heap_mr_) {
    fprintf(stderr, "rdma_ep: ibv_reg_mr failed "
                    "for data buffer\n");
    return -1;
  }
  ...
  fprintf(stderr,
          "rdma_ep: Registered data buffer: addr=%p, size=%zu, "
          "lkey=0x%x, rkey=0x%x\n",
