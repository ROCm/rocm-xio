...
static constexpr uint32_t STRIDE =
    sizeof(struct bnxt_re_sge);              /* 16 */
static constexpr uint32_t SQE_HDR_SZ =
    sizeof(struct bnxt_re_bsqe)
    + sizeof(struct bnxt_re_send);           /* 32 */
static constexpr uint32_t CQE_SZ =
    sizeof(struct bnxt_re_req_cqe)
    + sizeof(struct bnxt_re_bcqe);           /* 32 */
static constexpr uint32_t MSN_ENTRY_SZ =
    sizeof(struct bnxt_re_msns);             /* 8  */
    ...
    uint32_t max_inline_data)
{
  ...
  if (max_inline_data > 0) {
    uint32_t il_aligned =
        align_up(max_inline_data, STRIDE);
    uint32_t cal = SQE_HDR_SZ + il_aligned;
    if (cal > s.wqe_sz) s.wqe_sz = cal;
  ...
  s.npsn = static_cast<uint32_t>(
      roundup_pow2(s.slots));
  s.psn_sz = MSN_ENTRY_SZ;

  uint32_t pg = static_cast<uint32_t>(
...
inline uint32_t cqe_size() { return CQE_SZ; }
