...
struct CopyLinearPacket
{
   ...
   SDMA_HOST_DEVICE SDMA_FORCEINLINE explicit CopyLinearPacket(void* src, void* dst, size_t size)
   {
      assert(src != nullptr && dst != nullptr && "CopyLinearPacket: nullptr address");
      assert(size > 0 && size <= 0x100000000ull && "CopyLinearPacket: size out of range");
      ...
      value.SRC_ADDR_HI_UNION.src_addr_63_32 = static_cast<uint32_t>(src_addr >> 32);

      uintptr_t dst_addr = reinterpret_cast<uintptr_t>(dst);
      value.DST_ADDR_LO_UNION.dst_addr_31_0 = static_cast<uint32_t>(dst_addr);
      value.DST_ADDR_HI_UNION.dst_addr_63_32 = static_cast<uint32_t>(dst_addr >> 32);
   }

   SDMA_HOST_DEVICE SDMA_FORCEINLINE const SDMA_PKT_COPY_LINEAR* data() const { return &value; }
   SDMA_HOST_DEVICE SDMA_FORCEINLINE SDMA_PKT_COPY_LINEAR* data() { return &value; }
   SDMA_HOST_DEVICE SDMA_FORCEINLINE static constexpr size_t size_bytes() { return sizeof(SDMA_PKT_COPY_LINEAR); }
};
...
struct TimestampPacket
{
   ...
   SDMA_HOST_DEVICE SDMA_FORCEINLINE const SDMA_PKT_TIMESTAMP* data() const { return &value; }
   SDMA_HOST_DEVICE SDMA_FORCEINLINE SDMA_PKT_TIMESTAMP* data() { return &value; }
   SDMA_HOST_DEVICE SDMA_FORCEINLINE static constexpr size_t size_bytes() { return sizeof(SDMA_PKT_TIMESTAMP); }
};
...
struct LargeSubWindowCopyPacket
{
   ...
   SDMA_HOST_DEVICE SDMA_FORCEINLINE LargeSubWindowCopyPacket(void* srcBuf, void* dstBuf, uint32_t tile_width,
                                                              uint32_t tile_height, uint32_t src_buffer_pitch,
                                                              uint32_t dst_buffer_pitch, uint32_t src_x, uint32_t src_y,
                                                              uint32_t dst_x, uint32_t dst_y)
   {
      assert(srcBuf != nullptr && dstBuf != nullptr && "LargeSubWindowCopyPacket: nullptr buffer");
      assert(tile_width > 0 && tile_height > 0 && "LargeSubWindowCopyPacket: zero tile dimension");

      value.HEADER_UNION.op = SDMA_OP_COPY;
      value.HEADER_UNION.sub_op = SDMA_SUBOP_COPY_LINEAR_SUB_WINDOW;

      uintptr_t src_addr = reinterpret_cast<uintptr_t>(srcBuf);
      value.SRC_ADDR_LO_UNION.src_base_addr_31_0 = static_cast<uint32_t>(src_addr);
      value.SRC_ADDR_HI_UNION.src_base_addr_63_32 = static_cast<uint32_t>(src_addr >> 32);
      ...
      value.SRC_SLICE_PITCH_LO_UNION.src_slice_pitch_31_0 = static_cast<uint32_t>(src_slice_pitch & 0xFFFFFFFF);
      value.SRC_SLICE_PITCH_HI_UNION.src_slice_pitch_47_32 = static_cast<uint16_t>((src_slice_pitch >> 32) & 0xFFFF);

      uintptr_t dst_addr = reinterpret_cast<uintptr_t>(dstBuf);
      value.DST_ADDR_LO_UNION.dst_data_31_0 = static_cast<uint32_t>(dst_addr);
      value.DST_ADDR_HI_UNION.src_data_63_32 = static_cast<uint32_t>(dst_addr >> 32);
      ...
      value.DST_SLICE_PITCH_LO_UNION.dst_slice_pitch_31_0 = static_cast<uint32_t>(dst_slice_pitch & 0xFFFFFFFF);
      value.DST_SLICE_PITCH_HI_UNION.dst_slice_pitch_47_32 = static_cast<uint16_t>((dst_slice_pitch >> 32) & 0xFFFF);
   ...
   SDMA_HOST_DEVICE SDMA_FORCEINLINE const SDMA_PKT_LINEAR_LARGE_SUB_WINDOW_COPY* data() const { return &value; }
   SDMA_HOST_DEVICE SDMA_FORCEINLINE SDMA_PKT_LINEAR_LARGE_SUB_WINDOW_COPY* data() { return &value; }
...
{
   static_assert(std::is_integral_v<T>, "AtomicAddPacket requires integral type");
   static_assert(sizeof(T) == 4 || sizeof(T) == 8, "AtomicAddPacket supports 32- or 64-bit values");
   ...
   SDMA_HOST_DEVICE SDMA_FORCEINLINE AtomicAddPacket(T* ptr, T delta)
   {
      ...
      value.HEADER_UNION.operation = (sizeof(T) == 8) ? SDMA_ATOMIC_ADD64 : SDMA_ATOMIC_ADD32;
   ...
   SDMA_HOST_DEVICE SDMA_FORCEINLINE const SDMA_PKT_ATOMIC* data() const { return &value; }
   SDMA_HOST_DEVICE SDMA_FORCEINLINE SDMA_PKT_ATOMIC* data() { return &value; }
   SDMA_HOST_DEVICE SDMA_FORCEINLINE static constexpr size_t size_bytes() { return sizeof(SDMA_PKT_ATOMIC); }
};

template <typename T> struct PollRegmemPacket
{
   static_assert(std::is_integral_v<T>, "PollRegmemPacket requires integral type");
   static_assert(sizeof(T) == 4, "PollRegmemPacket supports 32-bit values only");

   SDMA_PKT_POLL_REGMEM value{};

   SDMA_HOST_DEVICE SDMA_FORCEINLINE PollRegmemPacket(T* flag_ptr, T expected_value, uint32_t interval = 10,
   ...
   SDMA_HOST_DEVICE SDMA_FORCEINLINE const SDMA_PKT_POLL_REGMEM* data() const { return &value; }
   SDMA_HOST_DEVICE SDMA_FORCEINLINE SDMA_PKT_POLL_REGMEM* data() { return &value; }
   SDMA_HOST_DEVICE SDMA_FORCEINLINE static constexpr size_t size_bytes()
   {
