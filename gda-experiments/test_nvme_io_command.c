...
int main(int argc, char** argv) {
  ...
  struct nvme_command* sqes = mmap(NULL, sqe_size, PROT_READ | PROT_WRITE,
                                   MAP_SHARED, fd,
                                   NVME_GDA_MMAP_SQE * getpagesize());
  if (sqes == MAP_FAILED) {
    perror("mmap SQE");
    close(fd);
    return 1;
  ...
  struct nvme_completion* cqes = mmap(NULL, cqe_size, PROT_READ | PROT_WRITE,
                                      MAP_SHARED, fd,
                                      NVME_GDA_MMAP_CQE * getpagesize());
  if (cqes == MAP_FAILED) {
    perror("mmap CQE");
  ...
  // Map doorbells
  void* doorbell_base = mmap(NULL, getpagesize(), PROT_READ | PROT_WRITE,
                             MAP_SHARED, fd,
