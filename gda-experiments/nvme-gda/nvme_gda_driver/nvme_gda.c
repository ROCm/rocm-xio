...
static long nvme_gda_ioctl(struct file* file, unsigned int cmd,
                           unsigned long arg);
 ...
 */
static void __iomem* nvme_doorbell(struct nvme_gda_dev* dev, u16 qid,
                                   bool is_sq) {
  u32 offset = 0x1000 +
               ((2 * qid + (is_sq ? 0 : 1)) * (4 << dev->doorbell_stride));
...
static struct nvme_gda_queue* nvme_gda_create_queue_internal(
  ...
  if (qsize < NVME_GDA_MIN_QUEUE_SIZE || qsize > NVME_GDA_MAX_QUEUE_SIZE ||
      !is_power_of_2(qsize)) {
  ...
  queue->sqes = dma_alloc_coherent(&dev->pdev->dev, qsize * NVME_SQE_SIZE,
                                   &queue->sqes_dma, GFP_KERNEL);
  if (!queue->sqes) {
  ...
  queue->cqes = dma_alloc_coherent(&dev->pdev->dev, qsize * NVME_CQE_SIZE,
                                   &queue->cqes_dma, GFP_KERNEL);
  if (!queue->cqes) {
  ...
  pr_info("Created queue %u: size=%u, sqes_dma=0x%llx, cqes_dma=0x%llx\n", qid,
          qsize, queue->sqes_dma, queue->cqes_dma);

  return queue;

err_free_sqes:
  dma_free_coherent(&dev->pdev->dev, qsize * NVME_SQE_SIZE, queue->sqes,
                    queue->sqes_dma);
err_free_queue:
  spin_lock_irqsave(&dev->queue_lock, flags);
  dev->queues[qid] = NULL;
...
static void nvme_gda_destroy_queue_internal(struct nvme_gda_dev* dev, u16 qid) {
  ...
  /* Free DMA memory */
  if (queue->sqes)
    dma_free_coherent(&dev->pdev->dev, queue->qsize * NVME_SQE_SIZE,
                      queue->sqes, queue->sqes_dma);

  if (queue->cqes)
    dma_free_coherent(&dev->pdev->dev, queue->qsize * NVME_CQE_SIZE,
 ...
 * PCI probe
 */
static int nvme_gda_pci_probe(struct pci_dev* pdev,
  ...
  pr_info("BAR0: phys=0x%llx, size=0x%llx, virt=%p\n", gda_dev->bar0_phys,
          gda_dev->bar0_size, gda_dev->bar0);
...
static int __init nvme_gda_init(void) {
  ...
  /* Create device */
  nvme_gda_device = device_create(nvme_gda_class, NULL, nvme_gda_devt, NULL,
                                  DRIVER_NAME "0");
