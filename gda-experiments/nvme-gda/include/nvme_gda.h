 ...
 */
nvme_gda_queue_userspace *nvme_gda_create_queue(nvme_gda_context *ctx, uint16_t qsize);
 ...
 */
int nvme_gda_get_device_info(nvme_gda_context *ctx, struct nvme_gda_device_info *info);

/**
 * Lock memory for GPU access using HSA
 * Similar to rocSHMEM's rocm_memory_lock_to_fine_grain
 */
int nvme_gda_lock_memory_to_gpu(void *ptr, size_t size, void **gpu_ptr, int gpu_id);
...
__device__ struct nvme_completion nvme_gda_wait_completion(
	nvme_gda_queue_userspace *queue,
	uint16_t command_id);
