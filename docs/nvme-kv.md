...
GPU-initiated NVMe **Key-Value** (Store / Retrieve) from `__device__` code, as an
additive mode on the existing block `nvme-ep` endpoint. Block behavior is
...
returned value length from the completion. So KV is a `driveEndpointKv()` sibling
of `driveEndpointSingle()` selected by `ioParams.kvOpcode`, plus a few host
fields. No new endpoint, no registry regen, no CMake or kernel-module change.
...
| Key length | CDW11 bits 7:0 | 1..16; options in bits 15:8 (0) |
...
`kvSqeSetup()` (`src/include/nvme-kv.h`) encodes the KV dwords; the caller sets
opcode/nsid/DPTR exactly like block I/O. Status `0x87` = key does not exist
(reported, not fatal); `0x86` = invalid key size; `0x85` = invalid value size.

## Files touched

- **`src/include/nvme-kv.h`** (new) — KV opcodes, status codes, `kvSqeSetup()`.
...
namespace. The bundled `examples/stage2_kv_rados_gpu.sh` exercises it end-to-end
against one such target — an SPDK vfio-user **NVMe-KV** subsystem backed by Ceph
RADOS — and runs a GPU Store+Retrieve:
...
NVMe-KV target — different transport or value backend — works unchanged from the
host side, as long as it presents a CSI=KV namespace.
...
- ✅ End-to-end GPU run verified: GPU `__device__` code issues KV Store then
  ...
  `/dev/nvme0`) — no block namespace required — so rocm-xio creates IO queues
  and targets the KV nsid.
