...
int main(int argc, char* argv[]) {
  ...
  if (fd < 0) {
    std::cerr << "Failed to open /dev/nvme-axiio: " << strerror(errno)
              << std::endl;
    return 1;
  }
  ...
  std::cout << "  SQ DMA (IOVA): 0x" << std::hex << qinfo.sq_dma_addr
            << std::dec << std::endl;
  ...
  void* doorbell_ptr = mmap(NULL, page_size, PROT_READ | PROT_WRITE, MAP_SHARED,
                            fd, 2 * page_size);
  ...
  volatile uint32_t* cpu_doorbell = (volatile uint32_t*)((char*)doorbell_ptr +
                                                         sq_offset);
  ...
  write_buf_info.size = ((NUM_LBAS * BLOCK_SIZE + page_size_val - 1) /
                         page_size_val) *
                        page_size_val;
  read_buf_info.size = ((NUM_RANDOM_READS * BLOCK_SIZE + page_size_val - 1) /
                        page_size_val) *
                       page_size_val;

  std::cout << "Allocating write buffer (" << write_buf_info.size
            << " bytes)..." << std::endl;
  if (ioctl(fd, NVME_AXIIO_ALLOC_DMA, &write_buf_info) < 0) {
    std::cerr << "Failed to allocate write buffer: " << strerror(errno)
              << std::endl;
    ret = 1;
    goto cleanup;
  }

  std::cout << "Allocating read buffer (" << read_buf_info.size << " bytes)..."
            << std::endl;
  if (ioctl(fd, NVME_AXIIO_ALLOC_DMA, &read_buf_info) < 0) {
    std::cerr << "Failed to allocate read buffer: " << strerror(errno)
              << std::endl;
    ret = 1;
    goto cleanup;
  }

  // Map data buffers (offset 4 = first DMA buffer, offset 5 = second DMA
  // buffer) Note: mmap offset is in bytes, but vm_pgoff is in page units So we
  // use 4 * PAGE_SIZE for the first buffer, 5 * PAGE_SIZE for the second
  write_buf = mmap(NULL, write_buf_info.size, PROT_READ | PROT_WRITE,
                   MAP_SHARED, fd, 4 * page_size_val);
  read_buf = mmap(NULL, read_buf_info.size, PROT_READ | PROT_WRITE, MAP_SHARED,
                  fd, 5 * page_size_val);

  if (write_buf == MAP_FAILED || read_buf == MAP_FAILED) {
    std::cerr << "Failed to mmap data buffers: " << strerror(errno)
              << std::endl;
    ret = 1;
    goto cleanup;
  }
  ...
  hsa_amd_memory_lock(write_buf, write_buf_info.size,
                      &nvme_hsa_doorbell::gpu_agent, 1, &gpu_write_buf);
  hsa_amd_memory_lock(read_buf, read_buf_info.size,
                      &nvme_hsa_doorbell::gpu_agent, 1, &gpu_read_buf);

  std::cout << std::endl;
  std::cout << "=== Phase 1: Writing random data to LBAs 0-511 ==="
            << std::endl;
  ...
  for (uint32_t i = 0; i < NUM_LBAS; i++) {
    ...
    hipLaunchKernelGGL(write_random_pattern, dim3(1), dim3(512), 0, 0,
                       (uint8_t*)gpu_write_buf + (i * BLOCK_SIZE), (uint64_t)i,
                       seed);
  }
  ...
  *cpu_doorbell = 0; // Reset doorbell
  hipLaunchKernelGGL(generate_write_commands, dim3(1), dim3(1), 0, 0,
                     (struct nvme_sqe*)gpu_sq_ptr, gpu_doorbell, NSID, 0ULL,
                     NUM_LBAS, write_buf_info.dma_addr, qinfo.queue_size);
  if (hipDeviceSynchronize() != hipSuccess) {
  ...
  if (hipMalloc(&gpu_random_lbas, NUM_RANDOM_READS * sizeof(uint64_t)) !=
        hipSuccess ||
      hipMemcpy(gpu_random_lbas, random_lbas.data(),
                NUM_RANDOM_READS * sizeof(uint64_t),
  ...
  *cpu_doorbell = 0; // Reset doorbell
  hipLaunchKernelGGL(generate_random_read_commands, dim3(1), dim3(1), 0, 0,
                     (struct nvme_sqe*)gpu_sq_ptr, gpu_doorbell, NSID,
                     gpu_random_lbas, NUM_RANDOM_READS, read_buf_info.dma_addr,
                     qinfo.queue_size);
  if (hipDeviceSynchronize() != hipSuccess) {
  ...
  std::cout << "Verifying read data against expected LFSR sequence..."
            << std::endl;
  ...
  if (mismatches == 0) {
    ...
    std::cout << "  Verified " << NUM_RANDOM_READS << " random LBAs"
              << std::endl;
  } else {
