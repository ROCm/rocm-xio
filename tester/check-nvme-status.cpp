...
int main() {
  ...
  std::cout << "BAR0 Physical: 0x" << std::hex << info.bar0_phys << std::dec
            << std::endl;
  ...
  // Map BAR0
  void* bar0 = mmap(NULL, info.bar0_size, PROT_READ, MAP_SHARED, mem_fd,
                    info.bar0_phys);
  if (bar0 == MAP_FAILED) {
  ...
  std::cout << "  Doorbell Stride: " << ((cap >> 32) & 0xF) << " (4-byte units)"
            << std::endl;
  ...
  std::cout << " (Version: " << ((vs >> 16) & 0xFFFF) << "." << (vs & 0xFFFF)
            << ")" << std::endl;
  ...
  std::cout << "  Enable (EN):     " << (cc & 1 ? "YES ✅" : "NO ❌")
            << std::endl;
  std::cout << "  I/O Queue Sizes: SQE=" << (1 << ((cc >> 16) & 0xF))
            << " bytes, CQE=" << (1 << ((cc >> 20) & 0xF)) << " bytes"
            << std::endl;
  ...
  std::cout << "  Ready (RDY):        " << (csts & 1 ? "YES ✅" : "NO ❌")
            << std::endl;
  std::cout << "  Fatal Error (CFS):  " << (csts & 2 ? "YES ❌" : "NO ✅")
            << std::endl;
