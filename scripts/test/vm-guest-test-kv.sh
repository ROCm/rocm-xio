...
#
# PHASE splits this script into stages so the host-side workflow can
# snapshot SPDK's own RPC-visible state (scripts/test/spdk-kv-io-snapshot.sh)
# between them -- the guest has no Docker access, so it cannot take those
# snapshots itself; the host runs one ssh call per stage instead of one
# call for the whole test run. Default PHASE=all keeps the previous
# single-shot behaviour (build, load, run every kv-labeled test) for
# anyone invoking this script directly outside the CI workflow.
#
#   PHASE=setup    GPU bring-up, controller discovery, build, module load.
#                  Writes the discovered controller device to
#                  ${BUILD_DIR}/.kv-ctrl so later phases (new SSH
#                  sessions, no shared shell state) can read it back.
#   PHASE=test     Run one ctest stage. TEST_NAME_REGEX selects tests by
#                  name (ctest -R); TEST_EXCLUDE_REGEX excludes by name
#                  (ctest -E). Both apply on top of --label-regex
#                  "${CTEST_LABEL}", same as before.
#   PHASE=all      (default) setup, then every kv-labeled test in one
#                  ctest invocation, exactly as this script used to.
#
...
#   TEST_NAME_REGEX    ctest -R value (PHASE=test only)
#   TEST_EXCLUDE_REGEX ctest -E value (PHASE=test only)
...
SKIP_GPU="${SKIP_GPU:-0}"
PHASE="${PHASE:-all}"
KV_CTRL_FILE="${BUILD_DIR}/.kv-ctrl"
...
run_setup() {
    ...
    # KV namespaces (CSI=2) are not attached as block devices by the Linux
    # NVMe driver, so there is no /dev/nvme0n<N> for them. Discover the
    # controller by finding the first NVMe controller device node.
    KV_CTRL=""
    ...
    # RelWithDebInfo is not a preference: -O0 device code never completes a
    # GPU dispatch on the emulated rocjitsu target (see vm-guest-test.sh for
    ...
    mkdir -p "${BUILD_DIR}"
    echo "${KV_CTRL}" > "${KV_CTRL_FILE}"
}

run_test_stage() {
    if [ ! -f "${KV_CTRL_FILE}" ]; then
        echo "ERROR: ${KV_CTRL_FILE} not found -- run PHASE=setup first" >&2
        exit 1
    fi
    KV_CTRL="$(cat "${KV_CTRL_FILE}")"
    cd "${BUILD_DIR}"

    ctest_args=(--label-regex "${CTEST_LABEL}" --output-on-failure --no-tests=error)
    [ -n "${TEST_NAME_REGEX:-}" ] && ctest_args+=(-R "${TEST_NAME_REGEX}")
    [ -n "${TEST_EXCLUDE_REGEX:-}" ] && ctest_args+=(-E "${TEST_EXCLUDE_REGEX}")

    banner "Running ctest stage (label=${CTEST_LABEL} -R=${TEST_NAME_REGEX:-<none>} -E=${TEST_EXCLUDE_REGEX:-<none>})"
    ...
    # Wrap each ctest stage with a wall-clock timeout to handle the case
...
case "${PHASE}" in
    ...
    all)
        run_setup
        TEST_NAME_REGEX="" TEST_EXCLUDE_REGEX="" run_test_stage
        ;;
    *)
        echo "ERROR: unrecognised PHASE '${PHASE}' (expected setup, test or all)" >&2
        exit 1
        ;;
esac
