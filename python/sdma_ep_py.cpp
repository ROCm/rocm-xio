...
void register_sdma_ep(nb::module_& m)
{
   ...
   m.def("put",
         [](int srcDevice, int dstDevice, int channelIdx, uintptr_t src, uintptr_t dst, size_t size) {
            ...
            handle.put(reinterpret_cast<void*>(src),
   ...
   m.def("put_tile_signal",
         [](int srcDevice, int dstDevice, int channelIdx, const sdma_ep::Tile& tile, uintptr_t dst_ptr,
            ...
            auto handle = sdma_ep::getHostHandle(srcDevice, dstDevice, channelIdx);
            if (flag_bits == 32) {
               handle.put_tile_signal(tile, reinterpret_cast<void*>(dst_ptr), dst_stride,
                                     reinterpret_cast<uint32_t*>(flag_ptr),
                                     static_cast<uint32_t>(flag_value));
            } else if (flag_bits == 64) {
               handle.put_tile_signal(tile, reinterpret_cast<void*>(dst_ptr), dst_stride,
   ...
   m.def("wait_flag_then_put",
         [](int srcDevice, int dstDevice, int channelIdx, uintptr_t flag_ptr, uint32_t expected_value,
            ...
            auto handle = sdma_ep::getHostHandle(srcDevice, dstDevice, channelIdx);
            if (flag_bits == 32) {
               handle.wait_flag_then_put(reinterpret_cast<uint32_t*>(flag_ptr), expected_value,
   ...
   m.def("wait_flag_then_put_tile",
         [](int srcDevice, int dstDevice, int channelIdx, uintptr_t flag_ptr, uint32_t expected_value,
            ...
            auto handle = sdma_ep::getHostHandle(srcDevice, dstDevice, channelIdx);
            if (flag_bits == 32) {
               handle.wait_flag_then_put_tile(reinterpret_cast<uint32_t*>(flag_ptr), expected_value,
                                             tile, reinterpret_cast<void*>(dst_ptr), dst_stride);
            } else {
   ...
   m.def("wait_flag_then_put_tiles",
         [](int srcDevice, int dstDevice, int channelIdx, uintptr_t flag_ptr, uint32_t expected_value,
            ...
            if (flag_bits == 32) {
               ...
               handle.wait_flag_then_put_tiles(reinterpret_cast<uint32_t*>(flag_ptr), expected_value,
                                              tiles, dst_ptrs, dst_strides);
            } else {
   ...
   m.def("signal",
         [](int srcDevice, int dstDevice, int channelIdx, uintptr_t flag_ptr, uint64_t value,
            ...
            auto handle = sdma_ep::getHostHandle(srcDevice, dstDevice, channelIdx);
            if (flag_bits == 32) {
               handle.signal(reinterpret_cast<uint32_t*>(flag_ptr), static_cast<uint32_t>(value));
            } else if (flag_bits == 64) {
